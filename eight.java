...
class eight
{
    ...
    public static int calculateCost(int initialM[][], int finalM[][])
    {
        ...
        for (int i1 = 0; i1 < N1; i1++)
            for (int j1 = 0; j1 < N1; j1++)
                if (initialM[i1][j1]!=0 && initialM[i1][j1] !=
                        finalM[i1][j1])
                    count1++;
        return count1;
    }
                             ...
                             int y1, int finalM[][])
    {
        ...
        PriorityQueue<Node> pq1 = new PriorityQueue<>(new
                comp());
        ...
        while(!pq1.isEmpty())
        {
            ...
            for (int i1 = 0; i1 < 4; i1++)
            {
                if (isSafe(min1.x1 + row1[i1], min1.y1 + col1[i1])>0)
                {
                    ...
                    Node child1 = newNode(min1.m, min1.x1, min1.y1,
                            min1.x1 + row1[i1],min1.y1 + col1[i1], min1.level1 + 1, min1);
